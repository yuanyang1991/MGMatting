...
class CutMask(object):
    ...
    def __call__(self, sample):
        if np.random.rand() < self.perturb_prob:
            ...
            mask[x:x + perturb_size_h, y:y + perturb_size_w] = mask[x1:x1 + perturb_size_h,
...
class DataGenerator_Instance_Wise(Dataset):
    def __init__(self, data, phase="train"):
        ...
        self.use_instance_wise = CONFIG.train.use_instance_wise
        if self.phase == "train":
            self.fg = data.fg
            self.bg = data.bg
            self.merged = []
    ...
    def _composite_fg(self, fg, alpha, idx):
        ...
        if np.random.rand() < 0.5:
            idx2 = np.random.randint(self.fg_num) + idx
            fg2_path = self.fg[idx2 % self.fg_num]
            # print(f"fg2 path {fg2_path} idx:{idx}")
            fg2 = cv2.imread(fg2_path)
            alpha2 = cv2.imread(self.alpha[idx2 % self.fg_num], 0).astype(np.float32) / 255.
            h, w = alpha.shape
            fg2 = cv2.resize(fg2, (w, h), interpolation=maybe_random_interp(cv2.INTER_NEAREST))
    ...
    def _composite_fg2(self, fg, alpha, bg, idx):
        # 1. 随机抽取第二张图片的alph和fg
        # 2. 将抽取到的alpha，fg 与 bg 融合
        # 3. 将本fg，alpha与上一步生成的图片融合
        # 4. 生成此时的fg1, alpha1, fg2, alpha2
        # 5. 随机选取一个((fg1, alpha1) / (fg2, alpha2), bg)返回

        # 问题：1， 按照上述方法返回的bg应该是哪个？ 2 是否有必要fg2, alpha2先与bg融合？
        return fg, alpha, bg
