...
class Trainer_Instance_Wise(object):
    ...
    def train(self):
        ...
        for step in range(start, self.train_config.total_step + 1):
            ...
            image, alpha, trimap, mask = image_dict['image'], image_dict['alpha'], image_dict['trimap'], image_dict[
                'mask']
            alpha2 = image_dict['another_alpha']
            fg2 = image_dict['another_fg']
            selected_index = image_dict['ramdom_number']
            alpha2 = alpha2.cuda()
            fg2 = fg2.cuda()
            ...
            if self.train_config.comp_weight > 0:
                self.loss_dict['comp'] = (self.composition_loss2(alpha_pred_os1, fg_norm, bg_norm, alpha2, fg2,
                                                                 selected_index, image,
                                                                 weight=weight_os1) * 2 + \
                                          self.composition_loss2(alpha_pred_os4, fg_norm, bg_norm, alpha2, fg2,
                                                                 selected_index, image,
                                                                 weight=weight_os4) * 1 + \
                                          self.composition_loss2(alpha_pred_os8, fg_norm, bg_norm, alpha2, fg2,
                                                                 selected_index, image,
    ...
    def mse(logit, target, weight):
        # return F.mse_loss(logit * weight, target * weight, reduction='sum') / (torch.sum(weight) + 1e-8)
        return Trainer_Instance_Wise.regression_loss(logit, target, loss_type='l2', weight=weight)
    ...
    @staticmethod
    def composition_loss2(alpha, fg, bg, alpha2, fg2, selected_index, image, weight, loss_type='l1'):
        batch_size = selected_index.size(0)
        merged_list = []

        for i in range(batch_size):
            if selected_index[i].item() == 0:
                new_bg = fg2[i] * alpha2[i] + bg[i] * (1 - alpha2[i])
                merged = fg[i] * alpha[i] + new_bg * (1 - alpha[i])
            else:
                new_bg = fg[i] * alpha[i] + bg[i] * (1 - alpha[i])
                merged = fg2[i] * alpha2[i] + new_bg * (1 - alpha2[i])
            merged_list.append(merged)

        merged = torch.stack(merged_list)
        return Trainer_Instance_Wise.regression_loss(merged, image, loss_type=loss_type, weight=weight)
    ...
    def gabor_loss(logit, target, gabor_filter, loss_type='l2', weight=None):
        ...
        return Trainer_Instance_Wise.regression_loss(gabor_logit, gabor_target, loss_type=loss_type, weight=weight)
    ...
    def grad_loss(logit, target, grad_filter, loss_type='l1', weight=None):
        ...
        return Trainer_Instance_Wise.regression_loss(grad_logit, grad_target, loss_type=loss_type, weight=weight)
    ...
    def lap_loss(logit, target, gauss_filter, loss_type='l1', weight=None):
        ...
        if weight is not None:
            ...
            return sum(Trainer_Instance_Wise.regression_loss(A[0], A[1], loss_type=loss_type, weight=A[2]) * (2 ** i) for i, A in
                       enumerate(zip(pyr_logit, pyr_target, pyr_weight)))
        else:
            return sum(Trainer_Instance_Wise.regression_loss(A[0], A[1], loss_type=loss_type, weight=None) * (2 ** i) for i, A in
