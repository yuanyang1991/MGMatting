...
def generator_tensor_dict2(image_path, mask_path):
    # read images
    image = cv2.imread(image_path)
    mask = cv2.imread(mask_path, 0)

    mask = (mask >= 128).astype(np.float32)  ### only keep FG part of trimap
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image-dir', type=str,
                        default='/export/ccvl12b/qihang/MGMatting/data/Combined_Dataset/Test_set/merged/',
                        help="input image dir")
    parser.add_argument('--mask-dir', type=str,
                        default='/export/ccvl12b/qihang/MGMatting/data/Combined_Dataset/Test_set/trimaps/',
                        help="input mask dir")
    ...
    if args.benchmark == "none":
        for i, image_name in enumerate(tqdm(os.listdir(args.image_dir))):
    ...
    elif args.benchmark == "him2k":
        for i, image_name in enumerate(tqdm(os.listdir(args.image_dir))):
            original_image_path = os.path.join(args.image_dir, image_name)
            base_name = os.path.splitext(image_name)[0]
            alpha_folder = os.path.join(args.mask_dir, base_name)
            alpha_images = os.listdir(alpha_folder)
            output_folder = os.path.join(args.output, base_name)
            os.makedirs(output_folder, exist_ok=True)
            for alpha_image in alpha_images:
                image_dict = generator_tensor_dict(original_image_path, os.path.join(alpha_folder, alpha_image),
                                                   args)
                alpha_pred = single_inference(model, image_dict, post_process=args.post_process)
                cv2.imwrite(os.path.join(output_folder, alpha_image), alpha_pred)
